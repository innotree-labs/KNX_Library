...
uint8_t build(PhysicalAddress source, uint16_t targetGa, const KnxValue& value,
              uint8_t* out, uint8_t maxLen, KNX_Priority priority, bool repeat) {
...
bool parse(const uint8_t* buf, uint8_t len, ParsedTelegram& out) {
	...
	out.priority      = (KNX_Priority)(buf[0] & 0x0C);
	...
	out.dpt = KNX_DPT::UNKNOWN;   // the receiving object supplies its DPT to decode
	return true;
}
